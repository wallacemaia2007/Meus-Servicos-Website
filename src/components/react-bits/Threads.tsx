...
const Threads = forwardRef<ThreadsHandle, ThreadsProps>(function Threads(
  {
    color = [1, 1, 1],
    amplitude = 1,
    distance = 0,
  ...
  // Scroll progress (0..1) driven externally (e.g. by a ScrollTrigger onUpdate)
  // through the imperative handle, so the shader uniforms can be updated without
  ...
  useEffect(() => {
    ...
    function update(t: number) {
      ...
      if (enableMouseInteraction) {
        ...
        // Scroll-driven: progress 0..1 maps to amplitude (0.3 -> 1.5) and to a
        // speed multiplier so the background "wakes up" slowly and gains energy.
        ...
        program.uniforms.uAmplitude.value =
          minAmplitude + (maxAmplitude - minAmplitude) * p;
        elapsed += delta * (minSpeed + (maxSpeed - minSpeed) * p);
      } else {
        program.uniforms.uMouse.value[0] = 0.5;
        program.uniforms.uMouse.value[1] = 0.5;
        program.uniforms.uAmplitude.value = amplitude;
